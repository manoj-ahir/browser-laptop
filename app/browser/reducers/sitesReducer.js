...
const sitesReducer = (state, action, emitChanges) => {
  switch (action.actionType) {
    case appConstants.APP_ADD_SITE:
      if (action.siteDetail.constructor === Immutable.List) {
        action.siteDetail.forEach((s) => {
          state = state.set('sites', siteUtil.addSite(state.get('sites'), s, action.tag))
        })
      } else {
        ...
        state = state.set('sites', siteUtil.addSite(sites, action.siteDetail, action.tag, action.originalSiteDetail))
      }
    ...
    case appConstants.APP_REMOVE_SITE:
      state = state.set('sites', siteUtil.removeSite(state.get('sites'), action.siteDetail, action.tag, true))
      break
